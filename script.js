...
function listTasks(filterParam) {
    let html = '';
    for (let i = 0; i < tasks.length; i++) {
        if (filterParam == tasks[i].status || filterParam == 'all' || filterParam == undefined) {
            html += `<div class="task-card" id="${tasks[i].id}">
...
function createNewTask(e) {
    if (e.keyCode == 13) {
        ...
        let obj = { description: e.target.value, status: 'active', isChecked: false, id: uniqueId() };
        tasks.push(obj);
        listTasks();
        console.log('last child(svg) ' + obj.id.lastChild);
...
// for (let i = 0; i < tasks.length; i++) {
//     let id = tasks[i].id;
//     console.log('adding listener' + tasks[i].id);
//     document.getElementById(id).lastChild.addEventListener('click', deleteTask);
// }
